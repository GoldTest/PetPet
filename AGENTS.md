...
- 禁止搜索 git 历史提交记录来寻找根因。所有分析基于当前代码内容。
- 用户明确说"不"或做出选择后，不得再反复建议相反的方案。尊重用户决策，在其约束下寻找解决方案。
...
- 日常打包默认只生成简名测试包：
  - Windows x64：`release/petpet<version>.exe`
  - Android arm64：`release/petpet<version>.apk`
...
- 全量打包产物命名：
  - Windows x64：`release/petpet<version>.exe`
  - Android arm64：`release/petpet<version>.apk`
  - Web 部署包：`release/petpet<version>-web.zip`
  - macOS 图形桌面包：`release/petpet<version>-mac.dmg`
  - Ubuntu/Linux 图形桌面包：优先 `release/petpet<version>-ubuntu.AppImage`，如 CI/runner 支持也保留 `release/petpet<version>-ubuntu.deb`
